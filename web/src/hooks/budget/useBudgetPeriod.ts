...
import type { BudgetPeriod, PeriodBounds, PeriodType } from '../../types/budget';
 ...
 * When the user navigates to a period that has no row yet, one is created with
 * income = 0.
 */
export function useBudgetPeriod(userId: string | null, type: PeriodType, bounds: PeriodBounds) {
  ...
  const setIncome = useCallback(
    async (income: number) => {
      setPeriod((prev) => (prev ? { ...prev, income } : prev));
      // Read the id from state via a functional update to avoid a stale closure.
